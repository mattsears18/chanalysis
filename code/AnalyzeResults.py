...
for name, group in groupedParticipant:
    ...
    participantAvgHullArea = getWeightedAverage(group, 'partAvgHullArea', 'partTime')
...
def plotAndSave(x, y, pointLabel, xLabel, yLabel, title, filename):    
    ...
    rLabel = plt.text(.98, .80, 'Pearson\'s r = ' + str(r)[0:7], horizontalalignment='right', verticalalignment='bottom', transform=ax.transAxes)
    pLabel = plt.text(.98, .75, 'p-value = ' + str(p)[0:6], horizontalalignment='right', verticalalignment='bottom', transform=ax.transAxes)
    nLabel = plt.text(.98, .70, 'n = ' + str(n), horizontalalignment='right', verticalalignment='bottom', transform=ax.transAxes)
